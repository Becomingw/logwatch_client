...
AI 在协助用户排查客户端行为时，需要遵守以下协议语义：

- `task_id` 由客户端为每次运行自动生成，必须全局唯一，不能复用旧任务 ID
- 如果服务端已经删除某个任务，服务端会保留 tombstone；该任务后续上报会收到 `409 task_deleted`
- 客户端收到 `task_deleted` 后应停止该任务继续推送；这是预期行为，不应建议“重试让任务复活”
- `task_not_running` 说明服务端认为该任务已不在运行态；这类问题应按状态竞争或收尾时序处理，而不是按鉴权失败处理
- 差网络导致的上传失败应视为可恢复问题；客户端会进入低频离线探测，网络恢复后可继续上传未送达日志

如果用户要重新开始监控，应重新执行命令，生成新的 `task_id`，而不是复用旧任务。
...
- 如果是验证“删除后客户端停止上报”，应让用户先启动任务、再在网页删除该任务，观察客户端出现 `task deleted`/归档提示，而不是期待旧任务恢复
...
1. 服务端的“删除任务”与“网络中断”不是同一种状态
2. 任务被服务端删除后，旧客户端继续推送被拒绝是正常的协议保护
3. 需要重跑任务时，应重新执行命令，生成新的 `task_id`
4. 如果只是网络差，优先检查客户端是否已进入低频离线探测，而不是直接判定为任务失败
